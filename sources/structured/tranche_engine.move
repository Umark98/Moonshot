...
module crux::tranche_engine {
    ...
    /// Admin capability for tranche settlement authorization
    ...
    public fun deposit_senior(
        ...
        // SECURITY: Require minimum junior deposit before allowing senior deposits
        // to prevent unlimited senior with zero first-loss buffer
        let new_senior = vault.senior_supply + sy_amount;
        assert!(vault.junior_supply > 0, EMaxLeverageExceeded);
        assert!(
    ...
    /// SECURITY: Requires AdminCap to prevent unauthorized settlement with fake yield.
    /// `actual_yield_sy` must be validated against on-chain SY exchange rate data
    /// by the caller (keeper/admin) before passing.
    public fun settle(
        _admin: &AdminCap,
        vault: &mut TrancheVault,
        actual_yield_sy: u64,
        ...
        // SECURITY: Cap yield at a reasonable maximum (200% of total deposited)
        // to prevent manipulation even with admin key compromise
        let max_yield = vault.total_sy_deposited * 2;
        let capped_yield = if (actual_yield_sy > max_yield) { max_yield } else { actual_yield_sy };

        vault.is_settled = true;
        vault.settlement_total_yield = capped_yield;
        ...
        if (capped_yield >= senior_target_yield) {
            ...
            let junior_yield = capped_yield - senior_target_yield;
            ...
            if (vault.senior_supply > 0) {
                vault.senior_payout_per_unit_wad = WAD + fixed_point::wad_div(
                    fixed_point::to_wad(capped_yield),
                    fixed_point::to_wad(vault.senior_supply),
                );
            };
